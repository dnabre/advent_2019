...
class IntCodeMachine:
    ...
    def lookup_value(self, p_mode, p_number):
        ...
        #	print(f'lookup {p_mode}{p_number}  position loc: {loc} value: {value}')
        elif (p_m == ParamMode.IMMEDIATE_MODE):
            value = self.program[self.pc + p_number]
        #	print(f'lookup {p_mode}{p_number}  immediate value: {value}')
        elif (p_m == ParamMode.RELATIVE_MODE):
            loc_adjust = self.program[self.pc + p_number] + self.relative_base
            value = self.program[loc_adjust]
        #	print(f'lookup {p_mode}{p_number}  relative loc_adjust: {loc_adjust} value: {value} relative_base: {self.relative_base}')
        else:
    ...
    def jump_if_true(self, p_modes):
        ...
        # if (num_1 != 0):
        if (num_1 != 0):
            print(f'jnz {num_1}, jmp-> {num_2}')
            self.pc = num_2
        else:
            self.pc += self.pc_shift[self.jump_if_true]
            print(f'jnz {num_1}, jmp-> {self.pc}')
        return
    ...
    def run_program(self):
        while True:
            ...
            pparam_modes, opcode_number = divmod(instruction, 100)

            operator = self.op_code[opcode_number]
            # print(f' pc:{self.pc}   opcode: {opcode_number}      param_modes: {pparam_modes} op: {operator.__name__} ')
            if (instruction in self.watch_list):
                self.watch = True
